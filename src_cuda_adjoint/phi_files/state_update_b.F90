...
!   gradient     of useful results: point.prim point.prim_old point.flux_res
!   with respect to varying inputs: point.prim point.prim_old point.flux_res
...
!   with respect to varying inputs: prim u
  SUBROUTINE PRIMITIVE_TO_CONSERVED_B(prim, primb, nx, ny, u, ub)
    IMPLICIT NONE
    REAL*8 :: rho, prim(4)
    REAL*8 :: rhob, primb(4)
    REAL*8 :: u(4), nx, ny
...
!   gradient     of useful results: point.delta point.prim
  ...
  SUBROUTINE FUNC_DELTA_B()
    ...
    DO i=max_points,1,-1
      ...
      DO r=ad_to,1,-1
        ...
        temp = pr/rho
        temp0 = DSQRT(temp)
        tempb = -(dist*delta_tb/(mod_u+3.0d0*temp0)**2)
        IF (temp .EQ. 0.0) THEN
          tempb0 = 0.0
        ELSE
          tempb0 = 3.0d0*tempb/(2.D0*DSQRT(temp)*rho)
        END IF
        mod_ub = tempb
        prb = tempb0
        rhob = -(temp*tempb0)
        ...
        IF (u1**2 + u2**2 .EQ. 0.0) THEN
          tempb1 = 0.0
        ELSE
          tempb1 = mod_ub/(2.D0*DSQRT(u1**2+u2**2))
        END IF
        u1b = 2*u1*tempb1
        u2b = 2*u2*tempb1
        CALL POPREAL8(dist)
...
!   gradient     of useful results: prim ubar
!   with respect to varying inputs: prim ubar
  SUBROUTINE CONSERVED_VECTOR_UBAR_B(prim, primb, ubar, ubarb, nx, ny)
    IMPLICIT NONE
    ...
    IF (pi*beta .EQ. 0.0) THEN
      betab = 0.0
    ELSE
      betab = -(EXP(-(s2**2))*pi*tempb3/(2.0*temp0**2))
    END IF
    IF (.NOT.beta .EQ. 0.0) betab = betab + u2_rot*s2b/(2.0*temp)
    tempb1 = betab/(2.0d0*pr)
    rhob = rhob + tempb1 - pr*tempb2/rho + a2p*ubarb(1)
    ubarb(1) = 0.0_8
    prb = tempb2 - rho*tempb1/pr
