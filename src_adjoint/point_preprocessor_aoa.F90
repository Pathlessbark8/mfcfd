...
#include <petsc/finclude/petscsys.h>
                ...
                allocate(pointd%x_res(2, max_points))
                allocate(pointd%nx(max_points))
                allocate(pointd%ny(max_points))
                
                allocate(pointdb%x(max_points))
                allocate(pointdb%y(max_points))
                allocate(pointdb%x_res(2, max_points))
                allocate(pointdb%nx(max_points))
                allocate(pointdb%ny(max_points))
                wall_points = 0
                interior_points = 0
                outer_points = 0
                shape_points = 0
