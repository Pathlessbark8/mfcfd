...
!  Tapenade 3.14 (r7079) -  5 Oct 2018 09:56
!
MODULE INTERIOR_FLUXES_MOD_DIFF_DIFF
...
CONTAINS
!  Differentiation of interior_dgx_pos_d in reverse (adjoint) mode (with options fixinterface):
...
!	This subroutine evaluates the interior flux derivative dGx_pos
  SUBROUTINE INTERIOR_DGX_POS_D_B(g, gb, gd, gdb, i)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: rho, u1, u2, pr
...
&       sum_delx_delfb)
      ...
      CALL FLUX_GXP_D_B(g_k, g_kb, g_kd, g_kdb, nx, nxb, ny, nyb, u1, &
...
&                            , prd, prdb)
      ...
      CALL FLUX_GXP_D_B(g_i, g_ib, g_id, g_idb, nx, nxb, ny, nyb, u1, &
&                 u1b, u1d, u1db, u2, u2b, u2d, u2db, rho, rhob, rhod, &
&                 rhodb, pr, prb, prd, prdb)
      CALL POPREAL8(u1)
      CALL POPREAL8(u1d)
...
&       tempb2) + SUM(point%dq(2, :, k)*tempb)
  ...
  END SUBROUTINE INTERIOR_DGX_POS_D_B
...
!  Differentiation of interior_dgx_neg_d in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: pointd.q pointd.dq pointd.qm
!                point.x point.y point.nx point.ny point.q point.dq
!                point.qm g gd
!   with respect to varying inputs: pointd.q pointd.dq pointd.qm
...
!	This subroutine evaluates the interior flux derivative dGx_neg
  SUBROUTINE INTERIOR_DGX_NEG_D_B(g, gb, gd, gdb, i)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: rho, u1, u2, pr
    REAL*8 :: rhob, u1b, u2b, prb
...
&       sum_delx_delfb)
      CALL POPREAL8ARRAY(g_k, 4)
      CALL POPREAL8ARRAY(g_kd, 4)
      CALL FLUX_GXN_D_B(g_k, g_kb, g_kd, g_kdb, nx, nxb, ny, nyb, u1, &
&                 u1b, u1d, u1db, u2, u2b, u2d, u2db, rho, rhob, rhod, &
&                 rhodb, pr, prb, prd, prdb)
      CALL POPREAL8(u1)
      CALL POPREAL8(u1d)
...
&                            , prd, prdb)
      CALL POPREAL8ARRAY(g_i, 4)
      CALL POPREAL8ARRAY(g_id, 4)
      CALL FLUX_GXN_D_B(g_i, g_ib, g_id, g_idb, nx, nxb, ny, nyb, u1, &
&                 u1b, u1d, u1db, u2, u2b, u2d, u2db, rho, rhob, rhod, &
&                 rhodb, pr, prb, prd, prdb)
      CALL POPREAL8(u1)
      CALL POPREAL8(u1d)
...
&       tempb2) + SUM(point%dq(2, :, k)*tempb)
    ...
    pointb%y(i) = pointb%y(i) + y_ib
    pointb%x(i) = pointb%x(i) + x_ib
  END SUBROUTINE INTERIOR_DGX_NEG_D_B
...
!  Differentiation of interior_dgy_pos_d in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: pointd.q pointd.dq pointd.qm
!                point.x point.y point.nx point.ny point.q point.dq
!                point.qm g gd
!   with respect to varying inputs: pointd.q pointd.dq pointd.qm
...
!
!
  SUBROUTINE INTERIOR_DGY_POS_D_B(g, gb, gd, gdb, i)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: rho, u1, u2, pr
    REAL*8 :: rhob, u1b, u2b, prb
...
&       tempb2) + SUM(point%dq(2, :, k)*tempb)
    ...
    pointb%y(i) = pointb%y(i) + y_ib
    pointb%x(i) = pointb%x(i) + x_ib
  END SUBROUTINE INTERIOR_DGY_POS_D_B
...
!  Differentiation of interior_dgy_neg_d in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: pointd.q pointd.dq pointd.qm
!                point.x point.y point.nx point.ny point.q point.dq
!                point.qm g gd
!   with respect to varying inputs: pointd.q pointd.dq pointd.qm
...
!   with respect to varying inputs: point.q point.dq point.qm
!	This subroutine evaluates the interior flux derivative dGx_neg
  SUBROUTINE INTERIOR_DGY_NEG_D_B(g, gb, gd, gdb, i)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: rho, u1, u2, pr
    REAL*8 :: rhob, u1b, u2b, prb
...
&       tempb2) + SUM(point%dq(2, :, k)*tempb)
    ...
    pointb%y(i) = pointb%y(i) + y_ib
    pointb%x(i) = pointb%x(i) + x_ib
  END SUBROUTINE INTERIOR_DGY_NEG_D_B
...
END MODULE INTERIOR_FLUXES_MOD_DIFF_DIFF
