...
&       point%dq(2, :, k))
      ...
      qtilde_id = pointd%q(:, i) - 0.5d0*(phi_id*(delx*point%dq(1, :, i)&
&       +dely*point%dq(2, :, i))+phi_i*(delxd*point%dq(1, :, i)+delx*&
&       pointd%dq(1, :, i)+delyd*point%dq(2, :, i)+dely*pointd%dq(2, :, &
&       i)))
      qtilde_i = point%q(:, i) - 0.5d0*phi_i*(delx*point%dq(1, :, i)+&
&       dely*point%dq(2, :, i))
      qtilde_kd = pointd%q(:, k) - 0.5d0*(phi_kd*(delx*point%dq(1, :, k)&
&       +dely*point%dq(2, :, k))+phi_k*(delxd*point%dq(1, :, k)+delx*&
&       pointd%dq(1, :, k)+delyd*point%dq(2, :, k)+dely*pointd%dq(2, :, &
&       k)))
...
&                          rho, rhod, pr, prd)
      CALL FLUX_QUAD_GXII_D(g_i, g_id, nx, nxd, ny, nyd, u1, u1d, u2, &
&                     u2d, rho, rhod, pr, prd)
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_k, qtilde_kd, u1, u1d, u2, u2d, &
&                          rho, rhod, pr, prd)
      CALL FLUX_QUAD_GXII_D(g_k, g_kd, nx, nxd, ny, nyd, u1, u1d, u2, &
&                     u2d, rho, rhod, pr, prd)
...
&       dely*point%dq(2, :, k))
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_i, qtilde_id, u1, u1d, u2, u2d, &
&                          rho, rhod, pr, prd)
      CALL FLUX_QUAD_GXI_D(g_i, g_id, nx, nxd, ny, nyd, u1, u1d, u2, u2d&
&                    , rho, rhod, pr, prd)
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_k, qtilde_kd, u1, u1d, u2, u2d, &
&                          rho, rhod, pr, prd)
      CALL FLUX_QUAD_GXI_D(g_k, g_kd, nx, nxd, ny, nyd, u1, u1d, u2, u2d&
&                    , rho, rhod, pr, prd)
      sum_delx_delfd = sum_delx_delfd + (g_kd-g_id)*dels_weights + (g_k-&
&       g_i)*dels_weightsd
      sum_delx_delf = sum_delx_delf + (g_k-g_i)*dels_weights
