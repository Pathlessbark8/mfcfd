...
!
    DO k=1,max_points
      ...
      beta = 0.5d0*rho/pr
      temp = u1*u1 + u2*u2
      pointd%q(1, k) = rhod/rho + (2.5d0/beta-temp)*betad - beta*(2*u1*&
&       u1d+2*u2*u2d)
      point%q(1, k) = DLOG(rho) + 2.5d0*DLOG(beta) - beta*temp
      two_times_betad = 2.0d0*betad
      two_times_beta = 2.0d0*beta
      pointd%q(2, k) = u1*two_times_betad + two_times_beta*u1d
      point%q(2, k) = two_times_beta*u1
      pointd%q(3, k) = u2*two_times_betad + two_times_beta*u2d
  ...
  SUBROUTINE EVAL_Q_DERIVATIVES_D()
    ...
    DO i=1,local_points
      ...
      DO k=1,point%nbhs(i)
        ...
        delyd = y_kd - y_id
        dely = y_k - y_i
        arg1d = 2*delx*delxd + 2*dely*delyd
...
&           power))) THEN
        ...
        END IF
        weights = dist**power
        sum_delx_sqrd = sum_delx_sqrd + weights*2*delx*delxd + delx**2*&
&         weightsd
        sum_delx_sqr = sum_delx_sqr + delx*delx*weights
        sum_dely_sqrd = sum_dely_sqrd + weights*2*dely*delyd + dely**2*&
&         weightsd
        sum_dely_sqr = sum_dely_sqr + dely*dely*weights
        sum_delx_delyd = sum_delx_delyd + weights*(dely*delxd+delx*delyd&
&         ) + delx*dely*weightsd
        sum_delx_dely = sum_delx_dely + delx*dely*weights
        sum_delx_delqd = sum_delx_delqd + (point%q(:, nbh)-point%q(:, i)&
&         )*(delx*weightsd+weights*delxd) + weights*delx*(pointd%q(:, &
&         nbh)-pointd%q(:, i))
        sum_delx_delq = sum_delx_delq + weights*delx*(point%q(:, nbh)-&
&         point%q(:, i))
        sum_dely_delqd = sum_dely_delqd + (point%q(:, nbh)-point%q(:, i)&
&         )*(dely*weightsd+weights*delyd) + weights*dely*(pointd%q(:, &
&         nbh)-pointd%q(:, i))
...
&       *sum_delx_dely*sum_delx_delyd
      ...
      temp1 = sum_delx_delq*sum_dely_sqr - sum_dely_delq*sum_delx_dely
      pointd%dq(1, :, i) = one_by_det*(sum_dely_sqr*sum_delx_delqd+&
&       sum_delx_delq*sum_dely_sqrd-sum_delx_dely*sum_dely_delqd-&
&       sum_dely_delq*sum_delx_delyd) + temp1*one_by_detd
      point%dq(1, :, i) = temp1*one_by_det
      temp1 = sum_dely_delq*sum_delx_sqr - sum_delx_delq*sum_delx_dely
      pointd%dq(2, :, i) = one_by_det*(sum_delx_sqr*sum_dely_delqd+&
&       sum_dely_delq*sum_delx_sqrd-sum_delx_dely*sum_delx_delqd-&
&       sum_delx_delq*sum_delx_delyd) + temp1*one_by_detd
      point%dq(2, :, i) = temp1*one_by_det
    END DO
...
&         ) + delx*dely*weightsd
        sum_delx_dely = sum_delx_dely + delx*dely*weights
        temp1d = q1d - 0.5d0*(point%dq(1, 1, i)*delxd+delx*pointd%dq(1, &
&         1, i)+point%dq(2, 1, i)*delyd+dely*pointd%dq(2, 1, i))
        temp1 = q1 - 0.5d0*(delx*point%dq(1, 1, i)+dely*point%dq(2, 1, i&
&         ))
        temp2d = pointd%q(1, nbh) - 0.5d0*(point%dq(1, 1, nbh)*delxd+&
&         delx*pointd%dq(1, 1, nbh)+point%dq(2, 1, nbh)*delyd+dely*&
&         pointd%dq(2, 1, nbh))
        temp2 = point%q(1, nbh) - 0.5d0*(delx*point%dq(1, 1, nbh)+dely*&
&         point%dq(2, 1, nbh))
        sum_delx_delq1d = sum_delx_delq1d + (temp2-temp1)*(delx*weightsd&
&         +weights*delxd) + weights*delx*(temp2d-temp1d)
        sum_delx_delq1 = sum_delx_delq1 + weights*delx*(temp2-temp1)
        sum_dely_delq1d = sum_dely_delq1d + (temp2-temp1)*(dely*weightsd&
&         +weights*delyd) + weights*dely*(temp2d-temp1d)
        sum_dely_delq1 = sum_dely_delq1 + weights*dely*(temp2-temp1)
        temp1d = q2d - 0.5d0*(point%dq(1, 2, i)*delxd+delx*pointd%dq(1, &
&         2, i)+point%dq(2, 2, i)*delyd+dely*pointd%dq(2, 2, i))
        temp1 = q2 - 0.5d0*(delx*point%dq(1, 2, i)+dely*point%dq(2, 2, i&
&         ))
        temp2d = pointd%q(2, nbh) - 0.5d0*(point%dq(1, 2, nbh)*delxd+&
&         delx*pointd%dq(1, 2, nbh)+point%dq(2, 2, nbh)*delyd+dely*&
&         pointd%dq(2, 2, nbh))
        temp2 = point%q(2, nbh) - 0.5d0*(delx*point%dq(1, 2, nbh)+dely*&
&         point%dq(2, 2, nbh))
        sum_delx_delq2d = sum_delx_delq2d + (temp2-temp1)*(delx*weightsd&
&         +weights*delxd) + weights*delx*(temp2d-temp1d)
        sum_delx_delq2 = sum_delx_delq2 + weights*delx*(temp2-temp1)
        sum_dely_delq2d = sum_dely_delq2d + (temp2-temp1)*(dely*weightsd&
&         +weights*delyd) + weights*dely*(temp2d-temp1d)
        sum_dely_delq2 = sum_dely_delq2 + weights*dely*(temp2-temp1)
        temp1d = q3d - 0.5d0*(point%dq(1, 3, i)*delxd+delx*pointd%dq(1, &
&         3, i)+point%dq(2, 3, i)*delyd+dely*pointd%dq(2, 3, i))
        temp1 = q3 - 0.5d0*(delx*point%dq(1, 3, i)+dely*point%dq(2, 3, i&
&         ))
        temp2d = pointd%q(3, nbh) - 0.5d0*(point%dq(1, 3, nbh)*delxd+&
&         delx*pointd%dq(1, 3, nbh)+point%dq(2, 3, nbh)*delyd+dely*&
&         pointd%dq(2, 3, nbh))
        temp2 = point%q(3, nbh) - 0.5d0*(delx*point%dq(1, 3, nbh)+dely*&
&         point%dq(2, 3, nbh))
        sum_delx_delq3d = sum_delx_delq3d + (temp2-temp1)*(delx*weightsd&
&         +weights*delxd) + weights*delx*(temp2d-temp1d)
        sum_delx_delq3 = sum_delx_delq3 + weights*delx*(temp2-temp1)
        sum_dely_delq3d = sum_dely_delq3d + (temp2-temp1)*(dely*weightsd&
&         +weights*delyd) + weights*dely*(temp2d-temp1d)
        sum_dely_delq3 = sum_dely_delq3 + weights*dely*(temp2-temp1)
        temp1d = q4d - 0.5d0*(point%dq(1, 4, i)*delxd+delx*pointd%dq(1, &
&         4, i)+point%dq(2, 4, i)*delyd+dely*pointd%dq(2, 4, i))
        temp1 = q4 - 0.5d0*(delx*point%dq(1, 4, i)+dely*point%dq(2, 4, i&
&         ))
        temp2d = pointd%q(4, nbh) - 0.5d0*(point%dq(1, 4, nbh)*delxd+&
&         delx*pointd%dq(1, 4, nbh)+point%dq(2, 4, nbh)*delyd+dely*&
&         pointd%dq(2, 4, nbh))
        temp2 = point%q(4, nbh) - 0.5d0*(delx*point%dq(1, 4, nbh)+dely*&
&         point%dq(2, 4, nbh))
        sum_delx_delq4d = sum_delx_delq4d + (temp2-temp1)*(delx*weightsd&
&         +weights*delxd) + weights*delx*(temp2d-temp1d)
        sum_delx_delq4 = sum_delx_delq4 + weights*delx*(temp2-temp1)
        sum_dely_delq4d = sum_dely_delq4d + (temp2-temp1)*(dely*weightsd&
&         +weights*delyd) + weights*dely*(temp2d-temp1d)
...
&       *sum_delx_dely*sum_delx_delyd
      ...
      one_by_detd = -(detd/det**2)
      one_by_det = 1.0d0/det
      temp3 = sum_delx_delq1*sum_dely_sqr - sum_dely_delq1*sum_delx_dely
      pointd%temp(1, 1, i) = temp3*one_by_detd + one_by_det*(&
&       sum_dely_sqr*sum_delx_delq1d+sum_delx_delq1*sum_dely_sqrd-&
&       sum_delx_dely*sum_dely_delq1d-sum_dely_delq1*sum_delx_delyd)
      point%temp(1, 1, i) = one_by_det*temp3
      temp3 = sum_delx_delq2*sum_dely_sqr - sum_dely_delq2*sum_delx_dely
      pointd%temp(1, 2, i) = temp3*one_by_detd + one_by_det*(&
&       sum_dely_sqr*sum_delx_delq2d+sum_delx_delq2*sum_dely_sqrd-&
&       sum_delx_dely*sum_dely_delq2d-sum_dely_delq2*sum_delx_delyd)
      point%temp(1, 2, i) = one_by_det*temp3
      temp3 = sum_delx_delq3*sum_dely_sqr - sum_dely_delq3*sum_delx_dely
      pointd%temp(1, 3, i) = temp3*one_by_detd + one_by_det*(&
&       sum_dely_sqr*sum_delx_delq3d+sum_delx_delq3*sum_dely_sqrd-&
&       sum_delx_dely*sum_dely_delq3d-sum_dely_delq3*sum_delx_delyd)
      point%temp(1, 3, i) = one_by_det*temp3
      temp3 = sum_delx_delq4*sum_dely_sqr - sum_dely_delq4*sum_delx_dely
      pointd%temp(1, 4, i) = temp3*one_by_detd + one_by_det*(&
&       sum_dely_sqr*sum_delx_delq4d+sum_delx_delq4*sum_dely_sqrd-&
&       sum_delx_dely*sum_dely_delq4d-sum_dely_delq4*sum_delx_delyd)
      point%temp(1, 4, i) = one_by_det*temp3
      temp3 = sum_dely_delq1*sum_delx_sqr - sum_delx_delq1*sum_delx_dely
      pointd%temp(2, 1, i) = temp3*one_by_detd + one_by_det*(&
&       sum_delx_sqr*sum_dely_delq1d+sum_dely_delq1*sum_delx_sqrd-&
&       sum_delx_dely*sum_delx_delq1d-sum_delx_delq1*sum_delx_delyd)
      point%temp(2, 1, i) = one_by_det*temp3
      temp3 = sum_dely_delq2*sum_delx_sqr - sum_delx_delq2*sum_delx_dely
      pointd%temp(2, 2, i) = temp3*one_by_detd + one_by_det*(&
&       sum_delx_sqr*sum_dely_delq2d+sum_dely_delq2*sum_delx_sqrd-&
&       sum_delx_dely*sum_delx_delq2d-sum_delx_delq2*sum_delx_delyd)
      point%temp(2, 2, i) = one_by_det*temp3
      temp3 = sum_dely_delq3*sum_delx_sqr - sum_delx_delq3*sum_delx_dely
      pointd%temp(2, 3, i) = temp3*one_by_detd + one_by_det*(&
&       sum_delx_sqr*sum_dely_delq3d+sum_dely_delq3*sum_delx_sqrd-&
&       sum_delx_dely*sum_delx_delq3d-sum_delx_delq3*sum_delx_delyd)
      point%temp(2, 3, i) = one_by_det*temp3
      temp3 = sum_dely_delq4*sum_delx_sqr - sum_delx_delq4*sum_delx_dely
      pointd%temp(2, 4, i) = temp3*one_by_detd + one_by_det*(&
&       sum_delx_sqr*sum_dely_delq4d+sum_dely_delq4*sum_delx_sqrd-&
&       sum_delx_dely*sum_delx_delq4d-sum_delx_delq4*sum_delx_delyd)
      point%temp(2, 4, i) = one_by_det*temp3
    END DO
...
&   rho, rhod, pr, prd)
    ...
    temp = 0.5d0/beta
    u1d = temp*q2d + q2*tempd
    u1 = q2*temp
    u2d = temp*q3d + q3*tempd
    u2 = q3*temp
    temp0 = u1*u1 + u2*u2
    temp1d = q1d + temp0*betad + beta*(2*u1*u1d+2*u2*u2d)
    temp1 = q1 + beta*temp0
    temp2d = temp1d - betad/((gamma-1)*beta)
    temp2 = temp1 - DLOG(beta)/(gamma-1)
    rhod = DEXP(temp2)*temp2d
    rho = DEXP(temp2)
    prd = temp*rhod + rho*tempd
    pr = rho*temp
