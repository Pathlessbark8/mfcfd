...
contains
        ...
        subroutine q_lskum(time)
                ...
                ! istat = cudaFuncSetAttribute(cal_flux_residual, cudaFuncAttributeMaxDynamicSharedMemorySize, 96000)
                ! if (istat /= 0) then
                !     print *, 'main: error setting cudaFuncAttributeMaxDynamicSharedMemorySize',cudaGetErrorString(istat)
                !     stop
                ! endif
