...
contains
        ...
        attributes(global) subroutine cal_flux_residual1(x_d, nx_d, flag_d, dist_d, nbhs_d, conn_d, &
                & xpos_nbhs_d, xneg_nbhs_d, ypos_nbhs_d, yneg_nbhs_d, xpos_conn_d, xneg_conn_d, &
                & ypos_conn_d, yneg_conn_d, prim_d, q_d, qm_d, dq_d, ddq_d, flux_res_d, phi1_d)

                implicit none

                ! device variables
                real*8 :: x_d(:,:), nx_d(:,:), dist_d(:), phi1_d(:,:)
                ...
                flux_res_d(1,i) = 0.0d0
                flux_res_d(2,i) = 0.0d0
                flux_res_d(3,i) = 0.0d0
                flux_res_d(4,i) = 0.0d0
...
!                       Higher order accuracy using q-variables ..
                    ...
                    call venkat_limiter(qtilde, share, i, threadid, blockd, delx, dely, q_d, qm_d, dq_d, ddq_d, nbhs_d, conn_d, x_d, dist_d, phi1_d)
                    if (flag_d(i) == 0) then
                        call flux_quad_GxII(share, threadid, blockd, a, nx, ny, u1, u2, rho, pr)
                    elseif (flag_d(i) == 1) then
                        call flux_Gxp(share, threadid, blockd, a, nx, ny, u1, u2, rho, pr)
                    elseif (flag_d(i) == 2) then
                        call flux_quad_GXIII(share, threadid, blockd, a, nx, ny, u1, u2, rho, pr)
                    end if

                    call venkat_limiter(qtilde, share, k, threadid, blockd, delx, dely, q_d, qm_d, dq_d, ddq_d, nbhs_d, conn_d, x_d, dist_d, phi1_d)
                    if (flag_d(i) == 0) then
            ...
            if (flag_d(i) == 0) then
                ...
                elseif (flag_d(i) == 2) then
                        ...
                        call interior_dGy_pos(flux_shared, share, qtilde, x_d, nx_d, nbhs_d, conn_d, ypos_nbhs_d, &
                                & ypos_conn_d, dist_d, q_d, qm_d, dq_d, ddq_d, phi1_d)
                ...
                flux_res_d(1,i) = flux_res_d(1,i) + flux_shared(threadId) 
                flux_res_d(2,i) = flux_res_d(2,i) + flux_shared(threadId + blockD * 1) 
                flux_res_d(3,i) = flux_res_d(3,i) + flux_shared(threadId + blockD * 2) 
                flux_res_d(4,i) = flux_res_d(4,i) + flux_shared(threadId + blockD * 3) 

                ! if (i==1) then
                !     print *, "Q ", q_d(1,i), q_d(2,i), q_d(3,i), q_d(4,i)
                !     print *, "DQ1 ", dq_d(1,1,i), dq_d(1,2, i), dq_d(1,3,i), dq_d(1,4,i)
                !     print *, "DQ2 ", dq_d(2,1,i), dq_d(2,2, i), dq_d(2,3,i), dq_d(2,4,i)
                !     print *, "FluxRes ", flux_res_d(1,i), flux_res_d(2,i), flux_res_d(3,i), flux_res_d(4,i)
                !     print *, "Prim ", prim_d(1,i), prim_d(2,i), prim_d(3,i), prim_d(4,i)
                ! end if
        
        end subroutine

        attributes(global) subroutine cal_flux_residual2(x_d, nx_d, flag_d, dist_d, nbhs_d, conn_d, &
                & xpos_nbhs_d, xneg_nbhs_d, ypos_nbhs_d, yneg_nbhs_d, xpos_conn_d, xneg_conn_d, &
                & ypos_conn_d, yneg_conn_d, prim_d, q_d, qm_d, dq_d, ddq_d, flux_res_d, phi1_d)
...
!                       Higher order accuracy using q-variables ..
        ...
        attributes(device) subroutine venkat_limiter(qtilde, share, k, threadid, blockd, delx, dely, q_d, qm_d, dq_d, ddq_d, nbhs_d, conn_d, x_d, dist_d, phi1_d)

                implicit none

                ! device variables
                real*8 :: q_d(:,:), x_d(:,:), dist_d(:), dq_d(:,:,:), ddq_d(:,:,:), qm_d(:,:,:), phi1_d(:,:)
                integer :: nbhs_d(:), conn_d(:,:)
                ! local variables
                integer :: r, k, i
                real*8 :: delx, dely
...
!     Expressions for the split fluxes ..	
                ...
                if(op == 0) share(threadid + blockd * 3) = rho*(temp2 + 0.5d0*temp1*B1) + share(threadid + blockd * 3)
                if(op == 1) share(threadid + blockd * 3) = rho*(temp2 + 0.5d0*temp1*B1) - share(threadid + blockd * 3)
...
!		Expressions for the split fluxes ..	
                ...
                temp1 = (6.0d0*pr_by_rho) + u_sqr
                if(op == 0) share(threadid + blockd * 3) = rho*(temp2 + 0.5d0*temp1*B2) + share(threadid + blockd * 3)
                if(op == 1) share(threadid + blockd * 3) = rho*(temp2 + 0.5d0*temp1*B2) - share(threadid + blockd * 3)
