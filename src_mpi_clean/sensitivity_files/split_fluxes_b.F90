...
!     Expressions for the split fluxes ..	
    ...
    gxpb(4) = 0.D0
    temp2b = tempb0
    temp1b = b1*0.5d0*tempb0
    b1b = temp1*0.5d0*tempb0
    pr_by_rhob = 6.0d0*temp1b
    u_sqrb = temp1b
    temp1 = 7.0d0*pr_by_rho + u_sqr
    tempb0 = a1pos*0.5d0*temp2b
    a1posb = ut*temp1*0.5d0*temp2b
    temp1b = ut*tempb0
    pr_by_rhob = pr_by_rhob + 7.0d0*temp1b
    u_sqrb = u_sqrb + temp1b
    temp1b = rho*gxpb(3)
    utb = temp1*tempb0 + un*a1pos*temp1b
    temp1 = ut*un*a1pos + un*b1
    rhob = rhob + temp1*gxpb(3)
    gxpb(3) = 0.D0
    unb = (ut*a1pos+b1)*temp1b + 2*un*u_sqrb
    temp1 = pr_by_rho + ut*ut
    temp2 = temp1*a1pos + ut*b1
    temp2b = rho*gxpb(2)
    tempb0 = rho*gxpb(1)
    a1posb = a1posb + ut*un*temp1b + temp1*temp2b + ut*tempb0
    b1b = b1b + un*temp1b + ut*temp2b + tempb0
    temp1b = a1pos*temp2b
    utb = utb + b1*temp2b + 2*ut*temp1b + a1pos*tempb0
...
!		Expressions for the split fluxes ..	
    ...
    a1negb = ut*temp1*0.5d0*temp2b
    temp1b = ut*tempb0
    pr_by_rhob = pr_by_rhob + 7.0d0*temp1b
    u_sqrb = u_sqrb + temp1b
    temp1b = rho*gxnb(3)
    utb = temp1*tempb0 + un*a1neg*temp1b
    temp1 = ut*un*a1neg - un*b1
    rhob = rhob + temp1*gxnb(3)
    gxnb(3) = 0.D0
    unb = (ut*a1neg-b1)*temp1b + 2*un*u_sqrb
    temp1 = pr_by_rho + ut*ut
    temp2 = temp1*a1neg - ut*b1
    temp2b = rho*gxnb(2)
    tempb0 = rho*gxnb(1)
    a1negb = a1negb + ut*un*temp1b + temp1*temp2b + ut*tempb0
    b1b = b1b - un*temp1b - ut*temp2b - tempb0
    temp1b = a1neg*temp2b
    utb = utb + 2*ut*temp1b - b1*temp2b + a1neg*tempb0
    pr_by_rhob = pr_by_rhob + temp1b
    ...
    IF (.NOT.beta .EQ. 0.0) betab = betab + ut*s1b/(2.D0*DSQRT(beta))
    tempb = 0.5d0*betab/pr
    rhob = rhob + temp2*gxnb(2) + (ut*a1neg-b1)*gxnb(1) + tempb - pr*&
&     pr_by_rhob/rho**2
    gxnb(2) = 0.D0
    gxnb(1) = 0.D0
    prb = pr_by_rhob/rho - rho*tempb/pr
...
!		Expressions for the split fluxes ..	
    ...
    gypb(4) = 0.D0
    temp2b = tempb0
    temp1b = b2*0.5d0*tempb0
    b2b = temp1*0.5d0*tempb0
    pr_by_rhob = 6.0d0*temp1b
    u_sqrb = temp1b
    temp1 = 7.0d0*pr_by_rho + u_sqr
    tempb0 = a2pos*0.5d0*temp2b
    a2posb = un*temp1*0.5d0*temp2b
    temp1b = un*tempb0
    pr_by_rhob = pr_by_rhob + 7.0d0*temp1b
    u_sqrb = u_sqrb + temp1b
    temp1b = rho*gypb(2)
    unb = temp1*tempb0 + ut*a2pos*temp1b
    temp1 = ut*un*a2pos + ut*b2
    rhob = rhob + temp1*gypb(2)
    gypb(2) = 0.D0
    utb = (un*a2pos+b2)*temp1b + 2*ut*u_sqrb
    temp1 = pr_by_rho + un*un
    temp2 = temp1*a2pos + un*b2
    temp2b = rho*gypb(3)
    tempb0 = rho*gypb(1)
    a2posb = a2posb + ut*un*temp1b + temp1*temp2b + un*tempb0
    b2b = b2b + ut*temp1b + un*temp2b + tempb0
    temp1b = a2pos*temp2b
    unb = unb + b2*temp2b + 2*un*temp1b + a2pos*tempb0
    pr_by_rhob = pr_by_rhob + temp1b
    ...
    CALL POPREAL8(s2)
    temp = DSQRT(pi*beta)
    tempb0 = 0.5d0*b2b/temp
    s2b = s2b - 2*s2*DEXP(-(s2**2))*tempb0
    IF (pi*beta .EQ. 0.0) THEN
      betab = 0.D0
    ELSE
      betab = -(pi*DEXP(-(s2**2))*tempb0/(2.D0*DSQRT(pi*beta)*temp))
    END IF
    temp = DSQRT(beta)
    unb = unb + 2*un*u_sqrb + temp*s2b
    IF (.NOT.beta .EQ. 0.0) betab = betab + un*s2b/(2.D0*DSQRT(beta))
    tempb = 0.5d0*betab/pr
    rhob = rhob + temp2*gypb(3) + (un*a2pos+b2)*gypb(1) + tempb - pr*&
&     pr_by_rhob/rho**2
    gypb(3) = 0.D0
    gypb(1) = 0.D0
    prb = pr_by_rhob/rho - rho*tempb/pr
...
!		Expressions for the split fluxes ..	
    ...
    u_sqrb = temp1b
    temp1 = 7.0d0*pr_by_rho + u_sqr
    tempb0 = a2neg*0.5d0*temp2b
    a2negb = un*temp1*0.5d0*temp2b
    temp1b = un*tempb0
    pr_by_rhob = pr_by_rhob + 7.0d0*temp1b
    u_sqrb = u_sqrb + temp1b
    temp1b = rho*gynb(2)
    unb = temp1*tempb0 + ut*a2neg*temp1b
    temp1 = ut*un*a2neg - ut*b2
    rhob = rhob + temp1*gynb(2)
    gynb(2) = 0.D0
    utb = (un*a2neg-b2)*temp1b + 2*ut*u_sqrb
    temp1 = pr_by_rho + un*un
    temp2 = temp1*a2neg - un*b2
    temp2b = rho*gynb(3)
    tempb0 = rho*gynb(1)
    a2negb = a2negb + ut*un*temp1b + temp1*temp2b + un*tempb0
    b2b = b2b - ut*temp1b - un*temp2b - tempb0
    temp1b = a2neg*temp2b
    unb = unb + 2*un*temp1b - b2*temp2b + a2neg*tempb0
    pr_by_rhob = pr_by_rhob + temp1b
    ...
    IF (.NOT.beta .EQ. 0.0) betab = betab + un*s2b/(2.D0*DSQRT(beta))
    tempb = 0.5d0*betab/pr
    rhob = rhob + temp2*gynb(3) + (un*a2neg-b2)*gynb(1) + tempb - pr*&
&     pr_by_rhob/rho**2
    gynb(3) = 0.D0
    gynb(1) = 0.D0
    prb = pr_by_rhob/rho - rho*tempb/pr
