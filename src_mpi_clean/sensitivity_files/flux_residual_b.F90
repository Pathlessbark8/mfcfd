...
!  Hint: ISIZE1OFDrfpoint_flux_res should be the size of dimension 1 of array *point%flux_res
    ...
    DO i=1,wall_points
      ...
      CALL PUSHREAL8ARRAY(point%flux_res(:, k), &
&                   ISIZE1OFDrfpoint_flux_res)
...
&       flux_res(:, k))
    ...
    DO i=wall_points,1,-1
      k = wall_points_index(i)
      CALL POPREAL8ARRAY(point%flux_res(:, k), ISIZE1OFDrfpoint_flux_res&
&                 )
      pointb%delta(k) = pointb%delta(k) + SUM(point%flux_res(:, k)*&
&       pointb%flux_res(:, k))*2.0d0
      pointb%flux_res(:, k) = point%delta(k)*2.0d0*pointb%flux_res(:, k)
      gxnb = 0.0_8
      gxpb = 0.0_8
      gynb = 0.0_8
      CALL POPREAL8ARRAY(point%flux_res(:, k), ISIZE1OFDrfpoint_flux_res&
