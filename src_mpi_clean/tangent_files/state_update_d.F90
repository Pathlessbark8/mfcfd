...
&                             u_oldd)
      ...
      u3_rotd = ud(3)
      u3_rot = u(3)
      ud(2) = u2_rotd*ny + u2_rot*nyd + u3_rotd*nx + u3_rot*nxd
      u(2) = u2_rot*ny + u3_rot*nx
      ud(3) = u3_rotd*ny + u3_rot*nyd - u2_rotd*nx - u2_rot*nxd
...
&     **2
    ...
    ud(2) = temp1d*ny + temp1*nyd - temp2d*nx - temp2*nxd
    u(2) = temp1*ny - temp2*nx
    ud(3) = temp1d*nx + temp1*nxd + temp2d*ny + temp2*nyd
...
! PetscErrorCode :: ierr
    DO i=1,local_points
      ...
      DO r=1,point%nbhs(i)
        ...
        y_kd = pointd%y(k)
        y_k = point%y(k)
        distd = (x_kd-x_id)*(x_k-x_i) + (x_k-x_i)*(x_kd-x_id) + (y_kd-&
&         y_id)*(y_k-y_i) + (y_k-y_i)*(y_kd-y_id)
...
&     u2_inf_rotd*u2_inf_rot + u2_inf_rot*u2_inf_rotd
    ...
    prd = primd(4)
    pr = prim(4)
    u1_rotd = u1d*tx + u1*txd + u2d*ty + u2*tyd
    u1_rot = u1*tx + u2*ty
    u2_rotd = u1d*nx + u1*nxd + u2d*ny + u2*nyd
    u2_rot = u1*nx + u2*ny
