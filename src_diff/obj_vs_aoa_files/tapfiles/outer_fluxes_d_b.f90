...
!  Tapenade 3.14 (r7079) -  5 Oct 2018 09:56
!
MODULE OUTER_FLUXES_MOD_DIFF_DIFF
!
...
CONTAINS
!  Differentiation of outer_dgx_pos_d in reverse (adjoint) mode (with options fixinterface):
...
!	This subroutine evaluates the wall flux derivative dGs_pos
  SUBROUTINE OUTER_DGX_POS_D_B(g, gb, gd, gdb, i)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: rho, u1, u2, pr
...
&                          rho, rhod, pr, prd)
      ...
      CALL FLUX_QUAD_GXIII_D(g_i, g_id, nx, ny, u1, u1d, u2, u2d, rho, &
&                      rhod, pr, prd)
      CALL PUSHREAL8(prd)
      CALL PUSHREAL8(pr)
      CALL PUSHREAL8(rhod)
...
&                          rho, rhod, pr, prd)
      ...
      CALL FLUX_QUAD_GXIII_D(g_k, g_kd, nx, ny, u1, u1d, u2, u2d, rho, &
&                      rhod, pr, prd)
...
&       sum_delx_delfb)
      ...
      CALL FLUX_QUAD_GXIII_D_B(g_k, g_kb, g_kd, g_kdb, nx, nxb, ny, nyb&
...
&                            , prd, prdb)
      ...
      CALL FLUX_QUAD_GXIII_D_B(g_i, g_ib, g_id, g_idb, nx, nxb, ny, nyb&
&                        , u1, u1b, u1d, u1db, u2, u2b, u2d, u2db, rho, &
&                        rhob, rhod, rhodb, pr, prb, prd, prdb)
      CALL POPREAL8(u1)
      CALL POPREAL8(u1d)
...
&       tempb2) + SUM(point%dq(2, :, k)*tempb)
  ...
  END SUBROUTINE OUTER_DGX_POS_D_B
...
!  Differentiation of outer_dgx_neg_d in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: pointd.q pointd.dq pointd.qm
!                point.x point.y point.nx point.ny point.q point.dq
!                point.qm g gd
!   with respect to varying inputs: pointd.q pointd.dq pointd.qm
...
!	This subroutine evaluates the wall flux derivative dGs_neg
  SUBROUTINE OUTER_DGX_NEG_D_B(g, gb, gd, gdb, i)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: rho, u1, u2, pr
    REAL*8 :: rhob, u1b, u2b, prb
...
&                          rho, rhod, pr, prd)
      CALL PUSHREAL8ARRAY(g_id, 4)
      CALL PUSHREAL8ARRAY(g_i, 4)
      CALL FLUX_QUAD_GXIV_D(g_i, g_id, nx, ny, u1, u1d, u2, u2d, rho, &
&                     rhod, pr, prd)
      CALL PUSHREAL8(prd)
      CALL PUSHREAL8(pr)
      CALL PUSHREAL8(rhod)
...
&                          rho, rhod, pr, prd)
      CALL PUSHREAL8ARRAY(g_kd, 4)
      CALL PUSHREAL8ARRAY(g_k, 4)
      CALL FLUX_QUAD_GXIV_D(g_k, g_kd, nx, ny, u1, u1d, u2, u2d, rho, &
&                     rhod, pr, prd)
      sum_delx_delfd = sum_delx_delfd + dels_weights*(g_kd-g_id)
      sum_delx_delf = sum_delx_delf + (g_k-g_i)*dels_weights
      sum_dely_delfd = sum_dely_delfd + deln_weights*(g_kd-g_id)
...
&       sum_delx_delfb)
      CALL POPREAL8ARRAY(g_k, 4)
      CALL POPREAL8ARRAY(g_kd, 4)
      CALL FLUX_QUAD_GXIV_D_B(g_k, g_kb, g_kd, g_kdb, nx, nxb, ny, nyb, &
&                       u1, u1b, u1d, u1db, u2, u2b, u2d, u2db, rho, &
&                       rhob, rhod, rhodb, pr, prb, prd, prdb)
      CALL POPREAL8(u1)
      CALL POPREAL8(u1d)
...
&                            , prd, prdb)
      CALL POPREAL8ARRAY(g_i, 4)
      CALL POPREAL8ARRAY(g_id, 4)
      CALL FLUX_QUAD_GXIV_D_B(g_i, g_ib, g_id, g_idb, nx, nxb, ny, nyb, &
&                       u1, u1b, u1d, u1db, u2, u2b, u2d, u2db, rho, &
&                       rhob, rhod, rhodb, pr, prb, prd, prdb)
      CALL POPREAL8(u1)
      CALL POPREAL8(u1d)
...
&       tempb2) + SUM(point%dq(2, :, k)*tempb)
    ...
    pointb%y(i) = pointb%y(i) + y_ib
    pointb%x(i) = pointb%x(i) + x_ib
  END SUBROUTINE OUTER_DGX_NEG_D_B
...
!  Differentiation of outer_dgy_pos_d in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: pointd.q pointd.dq pointd.qm
!                point.x point.y point.nx point.ny point.q point.dq
!                point.qm g gd
!   with respect to varying inputs: pointd.q pointd.dq pointd.qm
...
!   variations   of useful results: g
!   with respect to varying inputs: point.q point.dq point.qm
  SUBROUTINE OUTER_DGY_POS_D_B(g, gb, gd, gdb, i)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: rho, u1, u2, pr
    REAL*8 :: rhob, u1b, u2b, prb
...
&       sum_delx_delfb)
      CALL POPREAL8ARRAY(g_k, 4)
      CALL POPREAL8ARRAY(g_kd, 4)
      CALL FLUX_GYP_D_B(g_k, g_kb, g_kd, g_kdb, nx, nxb, ny, nyb, u1, &
...
&                            , prd, prdb)
      CALL POPREAL8ARRAY(g_i, 4)
      CALL POPREAL8ARRAY(g_id, 4)
      CALL FLUX_GYP_D_B(g_i, g_ib, g_id, g_idb, nx, nxb, ny, nyb, u1, &
&                 u1b, u1d, u1db, u2, u2b, u2d, u2db, rho, rhob, rhod, &
&                 rhodb, pr, prb, prd, prdb)
      CALL POPREAL8(u1)
      CALL POPREAL8(u1d)
...
&       tempb2) + SUM(point%dq(2, :, k)*tempb)
    ...
    pointb%y(i) = pointb%y(i) + y_ib
    pointb%x(i) = pointb%x(i) + x_ib
  END SUBROUTINE OUTER_DGY_POS_D_B
...
END MODULE OUTER_FLUXES_MOD_DIFF_DIFF
