...
!                point.prim_old point.flux_res
  SUBROUTINE STATE_UPDATE_B(rk)
    ...
    DO i=interior_points,1,-1
      ...
      IF (branch .EQ. 0) THEN
        pointb%flux_res(:, k) = pointb%flux_res(:, k) - 0.5d0*ub
      ELSE
...
&                             , nx, nxb, ny, nyb, u, ub)
    ...
    DO i=outer_points,1,-1
      ...
      temp = 1.0d0/u(1)
      CALL POPREAL8(point%prim(4, k))
      tempb1 = -(0.2d0*temp*pointb%prim(4, k))
      ub(4) = ub(4) + 0.4d0*pointb%prim(4, k)
      tempb = -(0.2d0*(u(2)**2+u(3)**2)*pointb%prim(4, k))
      ub(2) = ub(2) + 2*u(2)*tempb1
      pointb%prim(4, k) = 0.0_8
      ub(3) = ub(3) + temp*pointb%prim(3, k) + 2*u(3)*tempb1
      CALL POPREAL8(point%prim(3, k))
      tempb = tempb + u(3)*pointb%prim(3, k)
      pointb%prim(3, k) = 0.0_8
      CALL POPREAL8(point%prim(2, k))
...
&                            u, ub, nx, nxb, ny, nyb)
    ...
    DO i=wall_points,1,-1
      ...
      pointb%prim(4, k) = 0.0_8
      ub(3) = ub(3) + temp*pointb%prim(3, k) + 2*u(3)*tempb0
      CALL POPREAL8(point%prim(3, k))
      tempb = tempb + u(3)*pointb%prim(3, k)
      pointb%prim(3, k) = 0.0_8
      CALL POPREAL8(point%prim(2, k))
...
&   ub)
    ...
    temp1b = temp1b + 2*temp1*tempb
    temp2b = temp2b + 2*temp2*tempb
    ub(4) = 0.0_8
    rhob = prim(3)*temp2b + ub(1) + prim(2)*temp1b - (temp1**2+temp2**2)&
&     *tempb/rho
  ...
  SUBROUTINE FUNC_DELTA_B()
    ...
    DO i=local_points,1,-1
      ...
      DO r=ad_to,1,-1
        ...
        tempb0 = -(dist*tempb/(mod_u+3.0d0*temp0))
        IF (temp .EQ. 0.0) THEN
          tempb1 = 0.0
        ELSE
          tempb1 = 3.0d0*tempb0/(2.D0*DSQRT(temp)*rho)
        END IF
        distb = tempb
        mod_ub = tempb0
        prb = tempb1
        ...
        IF (u1**2 + u2**2 .EQ. 0.0) THEN
          tempb2 = 0.0
        ELSE
          tempb2 = mod_ub/(2.D0*DSQRT(u1**2+u2**2))
        END IF
        u1b = 2*u1*tempb2
        u2b = 2*u2*tempb2
        y_i = point%y(i)
        ...
        END IF
        CALL POPREAL8(dist)
        tempb3 = 2*(x_k-x_i)*distb
        tempb4 = 2*(y_k-y_i)*distb
        x_kb = tempb3
        x_ib = -tempb3
        y_kb = tempb4
        y_ib = -tempb4
...
&   ny, nyb)
    ...
    temp2b = ubarb(4)
    ubarb(4) = 0.0_8
    tempb = 0.5d0*b2*temp2b
    rhob = u2_rot*tempb + e*a2p*temp2b
    a2pb = e*rho*temp2b
    eb = rho*a2p*temp2b
    b2b = 0.5d0*rho*u2_rot*temp2b
    tempb1 = -(rho_inf*0.5d0*temp1b)
    a2n_infb = rho_inf*e_inf*temp1b
    e_infb = rho_inf*a2n_inf*temp1b
    temp1b = ubarb(3)
    temp2b = ubarb(3)
    ubarb(3) = 0.0_8
    tempb0 = rho*temp2b
    rhob = rhob + a2p*u1_rot*ubarb(2) + (u2_rot*a2p+b2)*temp2b
    a2pb = a2pb + rho*u1_rot*ubarb(2) + u2_rot*tempb0
    b2b = b2b + tempb0
    tempb2 = rho_inf*temp1b
    b2_infb = u2_inf_rot*tempb1 - tempb2
    a2n_infb = a2n_infb + rho_inf*u1_inf_rot*ubarb(2) + u2_inf_rot*&
&     tempb2
    u1_inf_rotb = rho_inf*a2n_inf*ubarb(2)
    ...
    s2b = 0.0_8
    temp0 = SQRT(pi*beta)
    tempb5 = b2b/(2.0d0*temp0)
    temp = SQRT(beta)
    tempb4 = eb/((gamma-1.0d0)*rho)
    temp1b = 0.5d0*eb
    u1_rotb = 2*u1_rot*temp1b + a2p*rho*ubarb(2)
    ubarb(2) = 0.0_8
    a2n_infb = a2n_infb + rho_inf*ubarb(1)
    a2pb = a2pb + rho*ubarb(1)
    result1b = 0.5d0*a2pb
    CALL DERF_B(s2, s2b, result1b)
    CALL POPREAL8(s2)
    s2b = s2b - EXP(-(s2**2))*2*s2*tempb5
    u2_rotb = a2p*tempb0 + 2*u2_rot*temp1b + temp*s2b + rho*tempb
    IF (pi*beta .EQ. 0.0) THEN
      betab = 0.0
    ELSE
      betab = -(EXP(-(s2**2))*pi*tempb5/(2.0*temp0**2))
    END IF
    IF (.NOT.beta .EQ. 0.0) betab = betab + u2_rot*s2b/(2.0*temp)
    tempb3 = betab/(2.0d0*pr)
    rhob = rhob + tempb3 - pr*tempb4/rho + a2p*ubarb(1)
    ubarb(1) = 0.0_8
    prb = tempb4 - rho*tempb3/pr
    ...
    CALL POPREAL8(s2)
    beta = 0.5d0*rho_inf/pr_inf
    s2b = s2b - DEXP(-(s2**2))*2*s2*b2_infb/(2.0d0*DSQRT(pi*beta))
    temp1b = 0.5d0*e_infb
    u2_inf_rotb = a2n_inf*tempb2 + 2*u2_inf_rot*temp1b + DSQRT(beta)*s2b&
&     + b2_inf*tempb1
    u1_inf_rotb = u1_inf_rotb + 2*u1_inf_rot*temp1b
    txb = u1_inf*u1_inf_rotb + u1*u1_rotb
