...
CONTAINS
!  Differentiation of max_q_value in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: point.q maxi
!   with respect to varying inputs: point.q
!	The following two subroutines are used for min-max limiter ..
  SUBROUTINE MAX_Q_VALUE_B(i, maxi, maxib)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: maxi(4)
    REAL*8 :: maxib(4)
...
!  Differentiation of min_q_value in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: point.q mini
!   with respect to varying inputs: point.q
  SUBROUTINE MIN_Q_VALUE_B(i, mini, minib)
    IMPLICIT NONE
    INTEGER :: i, j, k, r
    REAL*8 :: mini(4)
    REAL*8 :: minib(4)
...
! Denominator ..
    ...
    DO r=4,1,-1
      ...
      del_negb = den*denb
      denb = del_neg*denb
      del_posb = del_posb + 2.0d0*del_neg**2*numb + (2*del_pos+del_neg)*&
&       denb
      CALL POPREAL8(den)
      CALL POPREAL8(num)
